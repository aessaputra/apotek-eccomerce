...
import type { ProductsPerformanceSnapshot } from './useProductsPaginated';
...
function getProductsRequestKey(offset: number, limit: number): string {
  return `products:${ALL_PRODUCTS_CACHE_KEY}:${offset}:${limit}`;
}

function getProductsRequestPrefix(): string {
  return `products:${ALL_PRODUCTS_CACHE_KEY}:`;
}

export interface UseAllProductsPaginatedReturn {
...
export function useAllProductsPaginated(): UseAllProductsPaginatedReturn {
  const dispatch = useDispatch<Dispatch>();
  const cacheEntry = useSelector((state: State) => state.app.productsCache[ALL_PRODUCTS_CACHE_KEY]);
  ...
  const fetchProductsPage = useCallback(
    async ({ offset, replace, reason }: FetchProductsPageOptions): Promise<void> => {
      const requestKey = getProductsRequestKey(offset, PRODUCTS_PAGE_SIZE);
      const requestId = activeRequestIdRef.current + 1;
      activeRequestIdRef.current = requestId;

      if (replace) {
        cancelDedupedRequests(getProductsRequestPrefix());
      ...
      dispatch(
        appActions.setProductsCacheStatus({
          categoryId: ALL_PRODUCTS_CACHE_KEY,
  ...
  }, [cacheIsFresh, fetchProductsPage, products.length]);
...
export default useAllProductsPaginated;
